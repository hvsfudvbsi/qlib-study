...
- 当前区间：**2024-01-02 ~ 2026-09-17（658 个交易日）**，4 证券（3 股 + 上证指数），今日 K 线已包含
- 更新方式：全量重导（`--end` 改最新日期）+ `dump_all` 整体重建 + 重新拷贝 instruments；数据量增大后改用 `dump_update` 增量模式

## 实测记录（2026-09-17，v2）

- 首次验证时 3 只股票各 416 行，2024-01-02 ~ 2025-09-17（当期无停牌，416 = 日历天数）
