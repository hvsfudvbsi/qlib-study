...
## 2026 样本外复跑（2026-09-17，`run_oos2026.log`）

数据更新到 2026-09-17（658 天）后，切分改为 **train 2024 全年 / valid 2025 全年 / test 2026-01-01~09-15（真·完整样本外年度）**：

| 指标（test 2026，171 天） | 数值 | 对比 test 2025 |
|---|---|---|
| IC / ICIR | **+0.0359** / 0.0478 | -0.0090 / -0.0124 |
| Rank IC / Rank ICIR | +0.0638 / 0.0854 | 0.0066 / 0.0090 |
| 超额年化（含成本） | **-12.4%** | -10.5% |
| 超额最大回撤 | -23.2% | — |

**解读：IC 转正但超额仍亏**——IC 0.036 说明模型在 2026 年样本外学到了弱信号，但 3 只股票、topk=2 的组合只有 2 个持仓点，随机波动远大于微弱信号的贡献，加上双边成本直接把年化打到 -12%。样本过窄的问题不变：**先扩池（≥300 只）再谈收益**。

复跑过程修的两处（详见任务文档）：脚本头 `BENCHMARK` 行注释与 `TASK_SEGMENTS = {` 被合并成一行导致 IndentationError；重跑需带 `MLFLOW_ALLOW_FILE_STORE=true`。
...
| 区间 | 2024-01-02 ~ 2026-09-17（658 个交易日，2026-09-17 更新） |
| 股票池 | `stocks`（3 只）；benchmark `SH000001`（tx 源导出，factor 恒 1.0） |
| 模型 | Alpha158（158 特征）+ LGBM，切分见下方两轮复跑记录（当前：train 2024 / valid 2025 / test 2026-01~09） |
